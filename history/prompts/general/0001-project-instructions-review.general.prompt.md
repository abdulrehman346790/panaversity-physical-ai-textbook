...
user: unknown
command: /sp.phr
labels: [project-setup, instructions-review]
links:
  spec: null
  ticket: null
  adr: null
  pr: null
files:
- history/prompts/general/0001-project-instructions-review.general.prompt.md
tests:
- none
---
...
--title "Project Instructions Review" --stage general
...
If the user provided a task/question in --title "Project Instructions Review" --stage general:
...
- `{{PROMPT_TEXT}}` → **THE COMPLETE --title "Project Instructions Review" --stage general TEXT VERBATIM** (do NOT truncate to first line!)
...
**CRITICAL**: `{{PROMPT_TEXT}}` MUST be the FULL multiline user input from --title "Project Instructions Review" --stage general above, not just the title or first line.
...
Created Prompt History Record for initial project instructions review.

## Outcome

- ✅ Impact: Successfully recorded the initial project instructions review as a PHR.
- 🧪 Tests: none
- 📁 Files: 1 file created
- 🔁 Next prompts: none
- 🧠 Reflection: The PHR creation process needs to be robust for various user inputs.
...
- Next experiment (smallest change to try): Ensure full prompt text capture in PHR.
